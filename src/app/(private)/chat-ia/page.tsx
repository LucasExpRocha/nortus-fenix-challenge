...
import { MOCK_CHAT_IA } from '@/constants/chat-ia';
...
export default function ChatIa() {
  ...
  const chatData = data ?? MOCK_CHAT_IA;
  const [messages, setMessages] = useState<ChatIAMessage[]>(
    chatData.messages as ChatIAMessage[]
  );
  ...
  function sendQuickSuggestion() {
    const content = chatData.iaSuggestion;
    if (!content) return;
    setMessages((prev) => [
      ...prev,
      {
            ...
            {messages.map((message) => (
              ...
              >
                ...
                {message.type === 'ai_suggestion' &&
                  chatData.conversationAnalysis.futureSteps.actions.length >
                    0 && (
