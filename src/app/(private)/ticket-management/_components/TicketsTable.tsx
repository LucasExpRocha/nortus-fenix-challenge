...
function TicketsTableInner({
  ...
  useGSAP(() => {
    ...
    if (tbodyRef.current) {
      const tableRows = Array.from(tbodyRef.current.children);
  ...
  return (
    <Card className="gap-4">
      ...
      <div
        className="flex flex-wrap items-center gap-4 justify-between md:justify-end w-full px-6"
        ref={footerRef}
      >
  ...
  return (
    <section aria-label="Tabela de tickets" className="mt-6">
      <Suspense fallback={<TicketsTableSkeleton />}>
        <TicketsTableInner ticketsList={props.ticketsList} />
      </Suspense>
    </section>
  );
}
